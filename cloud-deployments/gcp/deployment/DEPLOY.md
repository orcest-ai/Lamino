...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create lamino-deployment --config gcp/deployment/gcp_deploy_lamino.yaml
...
```
gcloud compute instances get-serial-port-output lamino-instance 
```

ssh into the instance

```
gcloud compute ssh lamino-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete lamino-deployment 
```
