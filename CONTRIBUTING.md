...
4. **Security** is always important. If you have a security concern, please do not open an issue. Instead, please open a GitHub Security Advisory in this repository.
...
1. Unit test all bug fixes and new features. Your code will not be merged if it
   doesn't have tests.
1. If you change the public API, update the documentation accordingly.
...
The core library is written in Node.js. There are additional sub-repositories for the embed widget and browser extension. These are not part of the core Lamino project.
...
If you maintain a downstream desktop wrapper, keep it in a separate repository and periodically sync from the `master` branch.
