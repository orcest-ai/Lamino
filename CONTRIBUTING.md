# Contributing to Lamino

Lamino is an open-source project and we welcome contributions from the community.

## Reporting Issues

If you encounter a bug or have a feature request, please open an issue on the
[GitHub issue tracker](https://github.com/orcest-ai/lamino).

## Picking an issue

We track issues on the GitHub issue tracker. If you are looking for something to
work on, check the [good first issue](https://github.com/orcest-ai/lamino/contribute) label. These issues are typically the best described and have the smallest scope. There may be issues that are not labeled as good first issue, but are still a good starting point.
...
1. If you are modifying the permission system for a new role or something custom, you are likely better off forking the project and building your own version since this is a core part of the project and is only to be maintained by the Lamino team.
...
4. **Security** is always important. If you have a security concern, please do not open an issue. Instead, please open a CVE on our designated reporting platform [Huntr](https://huntr.com) or contact us at [team@orcest.ai](mailto:team@orcest.ai).
...
```bash
git clone https://github.com/<username>/lamino.git
cd lamino
```
...
```bash
git remote add upstream https://github.com/orcest-ai/lamino.git
git fetch upstream
```
...
1. Unit test all bug fixes and new features. Your code will not be merged if it
   doesn't have tests.
1. If you change the public API, update the documentation in the `lamino-docs` repository.
...
The core library is written in Node.js. There are additional sub-repositories for the embed widget and browser extension. These are not part of the core Lamino project, but are maintained by the Lamino team.
...
Changes to the core Lamino project are released through the `master` branch. When a PR is merged into `master`, a new version of the package is published to Docker and GitHub Container Registry under the `latest` tag.
...
Changes to the desktop app are downstream of the core Lamino project. Releases of the desktop app are published at the same time as the core Lamino project. Code from the core Lamino project is copied into the desktop app into an Electron wrapper. The Electron wrapper that wraps around the core Lamino project is **not** part of the core Lamino project, but is maintained by the Lamino team.

## License

By contributing to Lamino (this repository), you agree to license your contributions under the MIT license.