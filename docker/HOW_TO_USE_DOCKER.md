# How to use Dockerized Lamino

Use the Dockerized version of Lamino for a much faster and complete startup of Lamino.

### Minimum Requirements

> [!TIP]
> Running Lamino on AWS/GCP/Azure?
...
\*Lamino by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*Lamino by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized Lamino!
...
> the docker container using Lamino as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull orcestai/lamino
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run Lamino in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/lamino && \
...
$env:STORAGE_LOCATION="$HOME\Documents\lamino"; `
...
services:
  lamino:
    ...
    volumes:
      - lamino_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using Lamino! All your data and progress will persist between
...
- `git clone` this repo and `cd lamino` to get to the root directory.
- `touch server/storage/lamino.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of Lamino easier.

### Use the Midori AI Subsystem to Manage Lamino

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the Lamino docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in Lamino.
...
[Ask for help on Discord](https://orcest.ai)
