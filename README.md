...
- `yarn validate:enterprise:local` Run deterministic local enterprise validation (db reset + migrate + smoke).
  ...
  - Smoke summary now includes phase telemetry (`currentPhase`, `phaseHistory`) and `requestCount`; validator enforces `status=success`, `currentPhase=completed`, and `requestCount>0`.
...
- `yarn validate:enterprise:ci-local` Run the CI-equivalent enterprise pipeline locally (tests + OpenAPI drift check + frontend build + deterministic smoke).
  ...
  - CI workflow uploads server log + CI/stage + smoke/bootstrap summaries as artifacts for debugging failed runs.
