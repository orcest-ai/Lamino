# Run Lamino in production without Docker
...
Here you can find the scripts and known working process to run Lamino outside of a Docker container.
...
`git clone git@github.com:danialsamiei/llm.orcest.ai.git`

2. `cd lamino` and run `yarn setup`. This will install all dependencies to run in production as well as debug the application.
...
Lamino is comprised of three main sections. The `frontend`, `server`, and `collector`. When running in production you will be running `server` and `collector` on two different processes, with a build step for compilation of the frontend.
...
Lamino should now be running on `http://localhost:3001`!

## Updating Lamino

To update Lamino with future updates you can `git pull origin master` to pull in the latest code and then repeat steps 2 - 5 to deploy with all changes fully.
...
_note_ You should `pkill node` before running an update so that you are not running multiple Lamino processes on the same instance as this can cause conflicts.
...
cd $HOME/lamino/frontend && yarn && yarn build && cd $HOME/lamino
...
cd $HOME/lamino/collector && yarn

echo "Installing server dependencies & running migrations"
cd $HOME/lamino/server && yarn
cd $HOME/lamino/server && npx prisma migrate deploy --schema=./prisma/schema.prisma
cd $HOME/lamino/server && npx prisma generate

echo "Booting up services."
truncate -s 0 /logs/server.log # Or any other log file location.
truncate -s 0 /logs/collector.log

cd $HOME/lamino/server
(NODE_ENV=production node index.js) &> /logs/server.log &

cd $HOME/lamino/collector
