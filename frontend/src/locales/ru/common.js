...
const TRANSLATIONS = {
  onboarding: {
    ...
    llm: {
      ...
      description:
        "Lamino может работать с различными провайдерами LLM. Этот сервис будет обеспечивать обработку чата.",
    },
    userSetup: {
    ...
    survey: {
      title: "Добро пожаловать в Lamino",
      description:
        "Помогите нам сделать Lamino, созданным с учётом ваших потребностей (необязательно).",
      email: "Какой у вас адрес электронной почты?",
      useCase: "Для чего вы будете использовать Lamino?",
      useCaseWork: "Для работы",
      useCasePersonal: "Для личного использования",
      useCaseOther: "Другое",
      comment: "Откуда вы узнали о Lamino?",
      commentPlaceholder:
    ...
    workspace: {
      ...
      description:
        "Создайте ваше первое рабочее пространство и начните работу с Lamino.",
    },
  },
  common: {
  ...
  api: {
    title: "API ключи",
    description:
      "API ключи позволяют владельцу программно получать доступ к этому экземпляру Lamino и управлять им.",
  ...
  llm: {
    ...
    description:
      "Это учетные данные и настройки для вашего предпочтительного поставщика чата и встраивания LLM. Важно, чтобы эти ключи были актуальными и правильными, иначе Lamino не будет работать должным образом.",
  ...
  transcription: {
    ...
    "warn-start":
      "Использование локальной модели whisper на машинах с ограниченной оперативной памятью или процессором может привести к зависанию Lamino при обработке медиафайлов.",
    "warn-recommend":
  ...
  embedding: {
    ...
    "desc-end":
      "Встраивание - это процесс превращения текста в векторы. Эти учетные данные необходимы для превращения ваших файлов и подсказок в формат, который Lamino может использовать для обработки.",
    provider: {
  ...
  vector: {
    ...
    description:
      "Это учетные данные и настройки для того, как будет функционировать ваш экземпляр Lamino. Важно, чтобы эти ключи были актуальными и правильными.",
    provider: {
  ...
  privacy: {
    ...
    description:
      "Это ваша конфигурация для того, как подключенные сторонние поставщики и Lamino обрабатывают ваши данные.",
    llm: "Выбор LLM",
  ...
  connectors: {
    ...
    upload: {
      ...
      "privacy-notice":
        "Эти файлы будут загружены в процессор документов на этом экземпляре Lamino. Файлы не отправляются и не передаются третьим лицам.",
    },
    pinning: {
      what_pinning: "Что такое закрепление документа?",
      pin_explained_block1:
        "Когда вы <b>закрепляете</b> документ в Lamino, мы вставляем всё его содержимое в окно запроса, чтобы LLM мог полностью его понять.",
      pin_explained_block2:
        "Это работает лучше всего с <b>моделями с большим контекстом</b> или небольшими файлами, критичными для базы знаний.",
      pin_explained_block3:
        "Если по умолчанию ответы Lamino вас не удовлетворяют, закрепление — отличный способ получить более качественные ответы одним кликом.",
      accept: "Хорошо, понял",
    },
    watching: {
      what_watching: "Что делает функция наблюдения за документом?",
      watch_explained_block1:
        "Когда вы <b>наблюдаете</b> за документом в Lamino, мы <i>автоматически</i> синхронизируем его содержимое с оригинальным источником через регулярные интервалы. Это автоматически обновляет содержимое во всех рабочих пространствах, где используется этот файл.",
  ...
  customization: {
    interface: {
      ...
      description:
        "Настройте свои предпочтения пользовательского интерфейса для Lamino.",
    },
    branding: {
      title: "Брендинг и создание продуктов с собственной меткой.",
      description:
        "Настройте свою версию Lamino с использованием собственных брендинговых элементов.",
    },
    chat: {
      title: "Чат",
      description: "Настройте свои предпочтения для чата с Lamino.",
      auto_submit: {
    ...
    items: {
      ...
      "display-language": {
        ...
        description:
          "Выберите предпочитаемый язык для отображения пользовательского интерфейса Lamino – когда доступны переводы.",
      },
      logo: {
      ...
      "welcome-messages": {
        ...
        message: "сообщение",
        assistant: "Чат-ассистент Lamino",
  ...
  "main-page": {
    ...
    checklist: {
      ...
      completed:
        "Вы находитесь на пути к тому, чтобы стать экспертом в области Lamino!",
      dismiss: "закрыть",
      tasks: {
        create_workspace: {
    ...
    publish: {
      ...
      generic: {
        unauthenticated: {
          ...
          description:
            "Для публикации материалов необходимо сначала пройти аутентификацию в сообществе Lamino.",
  ...
  security: {
    ...
    password: {
      ...
      description:
        "Защитите ваш экземпляр Lamino паролем. Если вы забудете его, метода восстановления не существует, поэтому убедитесь, что вы сохранили этот пароль.",
