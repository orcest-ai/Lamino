...
# - manager user can still update non-enterprise preferences (e.g., `custom_app_name`) while enterprise flag writes are denied with explicit key-level error messages (including direct keys and `feature_flags` payload updates)
