...
# - default user denied admin API-key routes (e.g., `/admin/api-keys`, `/admin/generate-api-key`)
# - manager user can list/create teams
# - manager user is denied admin-only api key management routes (e.g., `/admin/api-keys`, `/admin/generate-api-key`, `/admin/api-keys/:id` update, and `/admin/delete-api-key/:id` delete)
