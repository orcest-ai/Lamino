...
- local validator enforces smoke summary invariants (`status=success`, `currentPhase=completed`, `requestCount>0`), required matrix-phase coverage in `phaseHistory`, and `verificationMatrix.status=pass` before reporting success
