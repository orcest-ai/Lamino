...
4. **Usage Policy Engine**
   ...
   - Effective-policy queries sanitize `teamIds` list inputs (drops invalid/decimal/non-positive entries) and dedupe normalized team IDs at resolver boundaries
   - Chat-policy limit parsing now enforces only strict positive numeric limits (invalid/blank/zero/negative values are ignored instead of causing accidental quota locks)
...
# - policy enforcement matrix includes maxPromptLength and maxTokensPerDay denial paths using scoped chat keys
