...
request "GET" "/admin/prompt-templates/999999/versions" "" "${TEAM_USER_TOKEN}"
assert_status "401" "default user denied prompt template version reads"
request "POST" "/admin/prompt-templates/999999/versions/new" "{\"prompt\":\"default denied version\"}" "${TEAM_USER_TOKEN}"
assert_status "401" "default user denied prompt template version creation"
request "POST" "/admin/prompt-templates/999999/versions/1/approve" "{}" "${TEAM_USER_TOKEN}"
assert_status "401" "default user denied prompt template version approval"
request "POST" "/admin/prompt-templates/999999/apply-to-workspace" "{\"workspaceId\":${WORKSPACE_ID}}" "${TEAM_USER_TOKEN}"
assert_status "401" "default user denied prompt template workspace apply"
...
request "GET" "/admin/prompt-templates/${MANAGER_TEMPLATE_ID}/versions" "" "${TEAM_USER_TOKEN}"
assert_status "401" "default user denied manager prompt template version reads"
request "POST" "/admin/prompt-templates/${MANAGER_TEMPLATE_ID}/apply-to-workspace" "{\"workspaceId\":${WORKSPACE_ID}}" "${TEAM_USER_TOKEN}"
assert_status "401" "default user denied manager prompt template apply"
...
assert_status "200" "manager can approve prompt template version"
if ! contains_text "$HTTP_BODY" "\"success\":true"; then
  log "FAILED: manager prompt template version approval did not return success payload."
  log "Response: ${HTTP_BODY}"
  exit 1
fi
...
request "POST" "/admin/usage-policies/${MANAGER_POLICY_ID}" "{\"name\":\"qa-default-denied-manager-policy-update-${RUN_ID}\"}" "${TEAM_USER_TOKEN}"
assert_status "401" "default user denied manager usage policy updates"
request "DELETE" "/admin/usage-policies/${MANAGER_POLICY_ID}" "" "${TEAM_USER_TOKEN}"
assert_status "401" "default user denied manager usage policy deletion"
...
request "POST" "/admin/api-keys/${WILDCARD_KEY_ID}" "{\"revokedAt\":\"2030-01-01T00:00:00.000Z\"}" "${ADMIN_TOKEN}"
assert_status "200" "revoke wildcard key"
request "GET" "/v1/auth" "" "${WILDCARD_KEY}"
assert_status "403" "revoked wildcard key denied on auth route"
if ! contains_text "$HTTP_BODY" "revoked"; then
  log "FAILED: revoked wildcard key auth denial missing expected message."
  log "Response: ${HTTP_BODY}"
  exit 1
fi
request "GET" "/v1/admin/teams" "" "${WILDCARD_KEY}"
assert_status "403" "revoked wildcard key denied on admin route"
if ! contains_text "$HTTP_BODY" "revoked"; then
  log "FAILED: revoked wildcard key admin denial missing expected message."
  log "Response: ${HTTP_BODY}"
  exit 1
fi
