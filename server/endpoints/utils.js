...
function getGitVersion() {
  if (process.env.LAMINO_RUNTIME === "docker") return "--";
  try {
...
function getModelTag() {
  ...
  switch (provider) {
    ...
    case "sambanova":
      ...
      break;
    case "rainymodel":
      model = process.env.RAINYMODEL_MODEL_PREF;
      break;
    default:
      model = "--";
      break;
...
/**
 * Returns the user agent for the Lamino deployment.
 * @returns {string} The user agent.
 */
function getLaminoUserAgent() {
  const version = getDeploymentVersion() || "unknown";
  return `Lamino/${version}`;
}
